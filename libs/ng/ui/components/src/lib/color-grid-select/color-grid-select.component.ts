...
import {AppService} from './../../../../../../../apps/ng-web/src/app/app.service'
  ...
  implements ControlValueAccessor, ColorGridSelect, AfterViewInit, OnDestroy
{
  ...
  ngOnChanges(){
    // console.log(this.selectedColor)
    this.appService.dataEmitter.subscribe((data)=>{
    //   console.log("data",data)
    //   this.selectedColor = this.textToRGB(data);
    // this.colorChange.emit(this.selectedColor);
      // console.log(this.textToRGB(data))
    })
  }

  textToRGB(text: string): string {
    let hash = 0;
